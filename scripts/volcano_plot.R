...
volcano_plot_f <- function(deseq_results, plot_out) {
    ...
    with(res, plot(log2FoldChange, -log10(pval), pch=20, main=comparisonName, col=rgb(1,1,1,0)))

    # Add colored points: red if padj<0.05, orange of log2FC>1, green if both)
    #significant = blue, non-sig = red, 30% alpha
    with(subset(res, padj<.05 ), points(log2FoldChange, -log10(pval), pch=20, col=rgb(70,131,180,75, maxColorValue=255))) #col="blue"))
    with(subset(res, padj>=.05), points(log2FoldChange, -log10(pval), pch=20, col=rgb(255,0,0,75,  maxColorValue=255)))
    ...
    topSig <- res[order(res$pval),]
    #print(head(topSig))
    with(head(topSig, 100), textxy(log2FoldChange, -log10(pval), labs=id, cex=.5))
