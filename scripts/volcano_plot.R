...
volcano_plot_f <- function(deseq_results, plot_out, png_file) {
    ...
    topSigPng <- res[order(res$pvalue),]
    with(head(topSigPng, 100), textxy(log2FoldChange, -log10(pvalue), labs=id, cex=.5))
    dev.off()
}
...
out = args[2]
png_file = args[3]
volcano_plot_f(deseq, out, png_file)
